...
def transformGenerateScenario(region, filename):
    ...
    airway = pd.read_json(
        'e:/ATMRI/CD_R/ATCO_preferences_interface/Scenario_interface_work/airways.json')

    # reassigned just for easy of duplicating from the original snippets (testingTheNeedOfFilteringAirways.ipynb)
    filteraws = airway
    ...
    for i in range(len(filteraws)):
        ...
        for j in range(len(filteraws.segments[i])):
            # print(j)
            P = np.array([float(filteraws.segments[i][j]['lon']),
                          float(filteraws.segments[i][j]['lat'])])
            vert_d = P[1]-a[1]
            horiz_d = P[0] - a[0]
            airdict1 = {
    ...
    keylist = ['sector1', 'sector2', 'sector3', 'sector4',
               'sector5', 'sector6', 'sector7', 'sector8']
    l1 = [[], [], [], [], [], [], [], []]
    for i in range(len(keylist)):
        for j in range(len(df2.Sectors[0][keylist[i]])):
            P = np.array([df2.Sectors[0][keylist[i]][j][0],
                          df2.Sectors[0][keylist[i]][j][1]])
            # print(P)
            vert_d = P[1]-a[1]
            horiz_d = P[0]-a[0]
            l1[i].append([(900/horizontal_len)*horiz_d,
                          900-(900/vertical_len)*vert_d])
    ...
    for i in range(10):  # for generating multiple scenario
        ...
        for i in range(len(df)):
            for j in range(len(df.segments[i])):
                waypointList.append(WayPoint(df.segments[i][j]['name'], [
                                    df.segments[i][j]['transf_x'], 900-df.segments[i][j]['transf_y']]))
        ...
        for i in range(ac_per_scenario):
            ...
            d = [20, 30, 40, 50, 60, 70, 80, 90, 100, 110, 120, 140,
                 150, 170, 190, 210, 220, 230, 240, 250]  # offset,
            # this randomization will create multiple scenaios
            aircraft = Aircraft(
                str(aircraft_name[i]), point, np.random.choice(d))
        ...
        l = ['sector1', 'sector2', 'sector3', 'sector4',
             'sector5', 'sector6', 'sector7', 'sector8']
        for i in l:
            if primarySector == i:
